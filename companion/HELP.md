...
If the connection says **Connected, but Teleprompter has not sent its document list**, the module has reached the app but needs a fresh protocol capture to determine the initial document-list request; do not enter a UUID manually. The module currently sends experimental mutations and reports only connection/discovery status; it has no live transport feedback yet. Keep a network key disabled during this experimental phase. See `docs/teleprompter-protocol.md` for capture and validation instructions.
