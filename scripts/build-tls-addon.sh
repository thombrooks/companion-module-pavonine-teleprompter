...
project_dir="$(pwd)"
build_dir="$(mktemp -d "$project_dir/.teleprompter-tls-build.XXXXXX")"
build_dir_relative="${build_dir#"$project_dir"/}"
...
"$project_dir/node_modules/.bin/pkg-prebuilds-copy" --baseDir "$build_dir_relative/arm64" --source teleprompter-tls-addon.node --name teleprompter-tls-addon --napi_version 10 --platform darwin --arch arm64
"$project_dir/node_modules/.bin/pkg-prebuilds-copy" --baseDir "$build_dir_relative/x86_64" --source teleprompter-tls-addon.node --name teleprompter-tls-addon --napi_version 10 --platform darwin --arch x64 --extraFiles libteleprompter_tls_native.dylib
