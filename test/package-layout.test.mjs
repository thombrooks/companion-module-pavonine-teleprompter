...
test('the native addon build uses pinned stable N-API headers under Node 22', () => {
	...
	assert.match(mainSource, /if \(timerStart !== undefined\) this\.documentTimerStarts\.set/)
	assert.doesNotMatch(mainSource, /if \(timerStart === undefined\) this\.documentTimerStarts\.delete/)
	assert.match(mainSource, /if \(!synchronized\) this\.reconnectKeyed\(\)/)
