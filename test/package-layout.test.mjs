...
test('the native addon build uses pinned stable N-API headers under Node 22', () => {
	...
	const addonSource = readFileSync(path.join(root, 'scripts/teleprompter_tls_addon.cpp'), 'utf8')
	assert.match(addonSource, /\{"close", nullptr, close_connection/)
	const mainSource = readFileSync(path.join(root, 'src/main.ts'), 'utf8')
	assert.match(mainSource, /Object\.hasOwn\(timerInfo, 'timerStart'\)/)
	assert.match(mainSource, /if \(isUnsetValue\(timerInfo\.timerStart\)\) this\.documentTimerStarts\.delete/)
	assert.match(mainSource, /if \(isUnsetValue\(timerStart\)\) this\.documentTimerStarts\.delete/)
	assert.match(
		mainSource,
		/manualShowElapsed\(this\.documentTimerStarts\.get\(documentId\), points\[0\]\?\.time, Date\.now\(\)\)/,
	)
	assert.match(mainSource, /timedShowTimerValues\(scheduled, scheduledTotal\)/)
	assert.match(mainSource, /Cached document/)
	assert.match(mainSource, /reconnect after Save/)
	assert.match(mainSource, /id: 'documentId'/)
	assert.match(mainSource, /if \(!this\.documents\.has\(this\.config\.documentId\)\) this\.reconnectKeyed\(\)/)
	assert.match(mainSource, /if \(this\.destroyed && this\.timerStatusTimer\) clearInterval/)
	assert.match(mainSource, /if \(documentId === this\.config\.documentId\) this\.updateDocumentStatus\(\)/)
	assert.match(mainSource, /\$\{ahead \? '' : '-'\}\$\{formatDuration\(Math\.abs\(values\.aheadBehind\)\)\}/)
	assert.match(mainSource, /size: segment \? '14' : 12/)
	assert.match(mainSource, /this\.documents\.clear\(\)/)
	assert.match(mainSource, /this\.documentTimingSnapshots\.clear\(\)/)
	assert.match(mainSource, /Restored saved document selection: \$\{name\}`\)\n\s*this\.updateSegmentStatus\(\)/)
	assert.match(mainSource, /Received selected-document motion update/)
	assert.match(mainSource, /Received selected-document motion snapshot/)
	assert.match(mainSource, /Play\/Pause feedback changed/)
})
