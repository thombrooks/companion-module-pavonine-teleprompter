...
test('an explicit CRDT unset remains distinguishable from an absent value', () => {
	assert.equal(isUnsetValue([1]), true)
	assert.equal(isUnsetValue([2, ['Delta', 0]]), false)
	assert.equal(isUnsetValue(undefined), false)
})
...
test('maximum observed index compares whole Delta values anywhere in an incoming tree message', () => {
	const message = [
		...
		[
			[
				['documents', 'doc', 'model', 'timing', 'motion'],
				[1, [[1], { index: ['+', 4, 9] }], false],
			],
			[
				['documents', 'doc', 'model', 'timing', 'manualSpeed'],
				[1, [[1], { index: ['+', 5, 1] }], false],
			],
		],
		0,
