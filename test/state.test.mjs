...
import { addressPreference, clampManualSpeed, documentStatus, estimateTiming, hasDifferentNetworkKey, networkKeyChallenge, preferredHosts, speedLabel } from '../dist/state.js'

test('discovery prefers IPv4, then global IPv6, then unusable link-local IPv6', () => {
	assert.deepEqual(preferredHosts(['fe80::1', '2001:db8::1', '192.168.1.14'], new Set()), ['192.168.1.14', '2001:db8::1', 'fe80::1'])
	assert.equal(addressPreference('fe80::1'), 2)
})

test('a local multi-NIC service uses loopback before advertised addresses', () => {
	assert.deepEqual(preferredHosts(['192.168.1.10', '10.0.0.10'], new Set(['10.0.0.10'])), ['::1', '127.0.0.1', '192.168.1.10', '10.0.0.10'])
})
...
test('estimated playhead preserves a middle position for pause and direction switching', () => {
	assert.deepEqual(estimateTiming({ keyPosition: 500, keyTime: 0.002 }, 'forward', 100, 1_000, 3_500), { keyPosition: 750, keyTime: 0.002 })
	assert.deepEqual(estimateTiming({ keyPosition: 500, keyTime: 0.002 }, 'reverse', 100, 1_000, 3_500), { keyPosition: 250, keyTime: 0.002 })
	assert.deepEqual(estimateTiming({ keyPosition: 50, keyTime: 0.002 }, 'reverse', 100, 1_000, 3_500), { keyPosition: 0, keyTime: 0.002 })
})
